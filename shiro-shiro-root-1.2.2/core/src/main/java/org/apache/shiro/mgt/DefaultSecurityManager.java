...
public class DefaultSecurityManager extends SessionsSecurityManager {
    ...
    public Subject createSubject(SubjectContext subjectContext) {
        ...
        if (log.isInfoEnabled()) {
            log.info("从context中解析session...");
        }
        context = resolveSession(context);
        if (log.isInfoEnabled()) {
            log.info("从context中解析session完毕。");
        }

        //Similarly, the SubjectFactory should not require any concept of RememberMe - translate that here first
        //if possible before handing off to the SubjectFactory:
        if (log.isInfoEnabled()) {
            log.info("从context中解析principals...");
        }
        context = resolvePrincipals(context);
        if (log.isInfoEnabled()) {
            log.info("从context中解析principals完毕。");
        }
        
        if (log.isInfoEnabled()) {
            log.info("根据解析好了的context构建subject。。。");
        }
        Subject subject = doCreateSubject(context);
        if (log.isInfoEnabled()) {
            log.info("根据解析好了的context构建subject完毕");
        }
        ...
        save(subject);
        if (log.isInfoEnabled()) {
            log.info("保存subject完毕，返回构建的subject。");
        }
    ...
    protected SubjectContext resolveSession(SubjectContext context) {
        if (context.resolveSession() != null) {
        	if (log.isInfoEnabled()) {
        		log.info("Context 已经包含一个session,返回。");
            }
            return context;
        }
        try {
            ...
            //the session manager:
        	if (log.isInfoEnabled()) {
        		log.info("Context自身没有session，尝试从session管理器中获取。。。");
            }
            Session session = resolveContextSession(context);
            if (log.isInfoEnabled()) {
        		log.info("从session管理器中获取完成。");
            }
            
            if(session==null){
            	if (log.isInfoEnabled()) {
            		log.info("session依然为null。");
                }
            }
            
            if (session != null) {
            	if (log.isInfoEnabled()) {
            		log.info("session不为null，将session注入到context中。");
                }
    ...
    protected Session resolveContextSession(SubjectContext context) throws InvalidSessionException {
    	if (log.isInfoEnabled()) {
    		log.info("从context中获取session。。。");
        }
    	if (log.isInfoEnabled()) {
    		log.info("首先从context中获取sessionKey。。。");
        }
        SessionKey key = getSessionKey(context);
        if (log.isInfoEnabled()) {
    		log.info("获取sessionKey完毕。");
        }
        
        if(key==null){
        	if (log.isInfoEnabled()) {
        		log.info("sessionKey未获取到，为null。");
            }
        }
        
        if (key != null) {
        	if (log.isInfoEnabled()) {
        		log.info("sessionKey获取到，不为null，根据key获取session。。。");
            }
            Session session =  getSession(key);
            if (log.isInfoEnabled()) {
        		log.info("从context中获取session完毕。");
            }
            if(session == null){
            	if (log.isInfoEnabled()) {
            		log.info("session为null，返回null");
                }
            }else{
            	if (log.isInfoEnabled()) {
            		log.info("session不为null，返回session。");
                }
            }
    ...
    protected SessionKey getSessionKey(SubjectContext context) {
    	if (log.isInfoEnabled()) {
    		log.info("从context的backmap中获取sessionId");
        }
        Serializable sessionId = context.getSessionId();
        if (sessionId != null) {
        	if (log.isInfoEnabled()) {
        		log.info("sessionId不为null，根据sessionId构建一个DefaultSessionKey并返回。");
            }
            return new DefaultSessionKey(sessionId);
        }
        if (log.isInfoEnabled()) {
    		log.info("未获取到sessionId返回null。");
        }
        return null;
    }
