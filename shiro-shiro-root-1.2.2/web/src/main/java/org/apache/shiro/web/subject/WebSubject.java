...
public interface WebSubject extends Subject, RequestPairSource {
    ...
    public static class Builder extends Subject.Builder {
        ...
        protected Builder setRequest(ServletRequest request) {
            if (request != null) {
            	if (log.isInfoEnabled()) {
                    log.info("当前类型为WebSubjectContext,设置ServletRequest属性为当前request");
                }
        ...
        protected Builder setResponse(ServletResponse response) {
            if (response != null) {
            	if (log.isInfoEnabled()) {
                    log.info("当前类型为WebSubjectContext,设置ServletResponse属性为当前response");
                }
        ...
        public WebSubject buildWebSubject() {
        	if (log.isInfoEnabled()) {
                log.info("调用父类构建subject...");
            }
            Subject subject = super.buildSubject();
            if (log.isInfoEnabled()) {
                log.info("调用父类构建subject完毕");
            }
            ...
            if (log.isInfoEnabled()) {
                log.info("返回subject(为WebSubject类型)");
            }
