...
public class DateUtil {
	...
	public static final ThreadLocal<SimpleDateFormat> LONG_DERLINE_DF = new ThreadLocal<SimpleDateFormat>() {
		protected SimpleDateFormat initialValue() {
	...
	/**
	 * 解析yyyy-MM-dd HH:mm:ss.SSS样式日期
	 * 
	 * @param time
	 * @return
	 */
	public static Date parseLongDerline(String time) {
		try {
			if(time.indexOf(".") > -1)
				return LONG_DERLINE_DF.get().parse(time);
			return DERLINE_DF.get().parse(time);
		} catch (ParseException e) {
			e.printStackTrace();
		}
	...
	public static String format3(Date time) {
		try {
			return LONG_DERLINE_DF.get().format(time);
		} catch (Exception e) {
			return null;
		}
	}	
	 ...
	 * System.out.println(convertToZeroDate(new Date()));
	 * System.out.println(DateUtils.addMinutes(DateUtil.convertToZeroDate(new
	 * Date()), 5)); System.out.println(Math.abs(100)); Date taskRunTime =
	 * DateUtils.addDays(DateUtils.addMinutes(DateUtil.convertToZeroDate(new
	 * Date()), 5), 1); System.out.println(DateUtil.getMInutesBetweenTwoDate(new
	 * Date(), taskRunTime) + 1); System.out.println(new Date().after(new
	 * Date())); System.out.println(DateUtil.getMorningAt8ByToday()); }
	 */
	
//	public static void main(String[] args) throws ParseException {
//		SimpleDateFormat format = new SimpleDateFormat("yyyyMMddHHmmss");
//		System.out.println(daysBetween(format.parse("20161201000000"), format.parse("20161202030000"), false));
//	}
}
