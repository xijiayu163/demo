...
public class ModularRealmAuthenticator extends AbstractAuthenticator {
    ...
    protected AuthenticationInfo doSingleRealmAuthentication(Realm realm, AuthenticationToken token) {
    	...
    	log.info("realm支持对当前类型的token进行授权");
        AuthenticationInfo info = realm.getAuthenticationInfo(token);
        log.info("realm根据token获取到的权限信息为:"+info);
        if (info == null) {
     ...
     * @throws IllegalStateException   if no realms have beent be authenticated or the user is denied authentication
     *                                 for the given princ configured at the time this method is invoked
     * @throws AuthenticationException if the user could noipal and credentials.
     */
