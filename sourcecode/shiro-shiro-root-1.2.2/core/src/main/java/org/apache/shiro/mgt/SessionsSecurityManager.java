...
public abstract class SessionsSecurityManager extends AuthorizingSecurityManager {
    ...
    public Session getSession(SessionKey key) throws SessionException {
    	if (log.isDebugEnabled()) {
    		log.debug("代理给sessionManager，根据sessionKey查找session。。。");
        }
    	
    	Session session = this.sessionManager.getSession(key);
    	if (log.isDebugEnabled()) {
    		log.debug("sessionManager查找session完毕，"+(session==null?"session 为null。":"session不为null。"));
        }
