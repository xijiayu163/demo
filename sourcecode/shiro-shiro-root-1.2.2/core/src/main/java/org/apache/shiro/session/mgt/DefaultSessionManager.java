...
public class DefaultSessionManager extends AbstractValidatingSessionManager implements CacheManagerAware {
    ...
    protected Session retrieveSession(SessionKey sessionKey) throws UnknownSessionException {
    	if (log.isDebugEnabled()) {
    		log.debug("根据sessionKey获取session。。。");
        }
    	
    	if (log.isDebugEnabled()) {
    		log.debug("根据sessionKey获取sessionId。。。");
        }
    	
        Serializable sessionId = getSessionId(sessionKey);
        if (sessionId == null) {
        	if (log.isDebugEnabled()) {
        		log.debug("sessionId为null，表示不创建当前session，返回null。");
            }
        ...
        if (log.isDebugEnabled()) {
    		log.debug("sessionId不为null，从数据源查找session。。。");
        }
        Session s = retrieveSessionFromDataSource(sessionId);
        if (log.isDebugEnabled()) {
    		log.debug("从数据源查找session完毕。");
        }
        if (s == null) {
        	if (log.isDebugEnabled()) {
        		log.debug("数据源未查找到指定id的session，抛出UnknownSessionException。");
            }
    ...
    protected Session retrieveSessionFromDataSource(Serializable sessionId) throws UnknownSessionException {
    	if (log.isDebugEnabled()) {
    		log.debug("从sessionDAO中查找session");
        }
