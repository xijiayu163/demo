...
public class DefaultSecurityManager extends SessionsSecurityManager {
    ...
    public Subject createSubject(SubjectContext subjectContext) {
    	...
    	if (log.isDebugEnabled()) {
            log.debug("开始构建subject...");
        }
    	if (log.isInfoEnabled()) {
            log.info("浅拷贝subjectContext");
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("从context中解析session...");
        }
        context = resolveSession(context);
        if (log.isDebugEnabled()) {
            log.debug("从context中解析session完毕。"+context.getSession());
        }

        //Similarly, the SubjectFactory should not require any concept of RememberMe - translate that here first
        //if possible before handing off to the SubjectFactory:
        if (log.isDebugEnabled()) {
            log.debug("从context中解析principals...");
        }
        context = resolvePrincipals(context);
        if (log.isDebugEnabled()) {
            log.debug("从context中解析principals完毕。");
        }
        
        if (log.isDebugEnabled()) {
            log.debug("根据解析好了的context构建subject。。。");
        }
        Subject subject = doCreateSubject(context);
        if (log.isDebugEnabled()) {
            log.debug("根据解析好了的context构建subject完毕");
        }
        ...
        //Added in 1.2:
        if (log.isInfoEnabled()) {
            log.info("保存subject，主要是合并principals和授权信息到session");
        }
        save(subject);
    ...
    protected SubjectContext resolveSession(SubjectContext context) {
        if (context.resolveSession() != null) {
        	if (log.isDebugEnabled()) {
        		log.debug("Context 已经包含一个session,返回。");
            }
            return context;
        }
        try {
            ...
            //the session manager:
        	if (log.isDebugEnabled()) {
        		log.debug("Context自身没有session，尝试从session管理器中获取。。。");
            }
            Session session = resolveContextSession(context);
            if (log.isDebugEnabled()) {
        		log.debug("从session管理器中获取完成。");
            }
            
            if(session==null){
            	if (log.isDebugEnabled()) {
            		log.debug("session依然为null。");
                }
            }
            
            if (session != null) {
            	if (log.isDebugEnabled()) {
            		log.debug("session不为null，将session注入到context中。");
                }
                context.setSession(session);
            }
        } catch (InvalidSessionException e) {
        	if (log.isInfoEnabled()) {
        		log.info("Resolved SubjectContext context session is invalid.  Ignoring and creating an anonymous " +
    ...
    protected Session resolveContextSession(SubjectContext context) throws InvalidSessionException {
    	if (log.isDebugEnabled()) {
    		log.debug("从context中获取session。。。");
        }
    	if (log.isDebugEnabled()) {
    		log.debug("首先从context中获取sessionKey。。。");
        }
        SessionKey key = getSessionKey(context);
        if (log.isDebugEnabled()) {
    		log.debug("获取sessionKey完毕。");
        }
        
        if(key==null){
        	if (log.isDebugEnabled()) {
        		log.debug("sessionKey未获取到，为null。");
            }
        }
        
        if (key != null) {
        	if (log.isDebugEnabled()) {
        		log.debug("sessionKey获取到，不为null，根据key获取session。。。");
            }
            Session session =  getSession(key);
            if (log.isDebugEnabled()) {
        		log.debug("从context中获取session完毕。");
            }
            if(session == null){
            	if (log.isDebugEnabled()) {
            		log.debug("session为null，返回null");
                }
            }else{
            	if (log.isDebugEnabled()) {
            		log.debug("session不为null，返回session。");
                }
            }
    ...
    protected SessionKey getSessionKey(SubjectContext context) {
    	if (log.isDebugEnabled()) {
    		log.debug("从context的backmap中获取sessionId");
        }
        Serializable sessionId = context.getSessionId();
        if (sessionId != null) {
        	if (log.isDebugEnabled()) {
        		log.debug("sessionId不为null，根据sessionId构建一个DefaultSessionKey并返回。");
            }
            return new DefaultSessionKey(sessionId);
        }
        if (log.isDebugEnabled()) {
    		log.debug("未获取到sessionId返回null。");
        }
        return null;
    }
    ...
    protected SubjectContext resolvePrincipals(SubjectContext context) {
        ...
        if (log.isInfoEnabled()) {
    		log.info("从当前上下文中解析principals信息为"+principals);
    		if(principals!=null){
    			log.info("principals不为null!!!!!!");
    		}
        }

        if (CollectionUtils.isEmpty(principals)) {
        	if (log.isInfoEnabled()) {
        		log.info("当前上下文没有PrincipalCollection(标识)被发现,尝试查找记住我的princials。。。");
            }
            ...
            if (!CollectionUtils.isEmpty(principals)) {
            	if (log.isInfoEnabled()) {
            		log.info("找到被记住的princials，添加到当前使用的context中。");
                }
                ...
                // bindPrincipalsToSession(principals, context);
            	 ...
            	 if (log.isInfoEnabled()) {
             		log.info("当前上下文没有记住我的princials,将context原样返回。");
                 }
            }
        }
