...
public class DefaultWebSessionManager extends DefaultSessionManager implements WebSessionManager {
    ...
    protected Session createExposedSession(Session session, SessionKey key) {
        ...
        if (log.isInfoEnabled()) {
    		log.info("根据sessionId，request,response 构建一个新的sessionKey");
        }
    ...
    public Serializable getSessionId(SessionKey key) {
    	...
    	if (log.isDebugEnabled()) {
    		log.debug("从父类中根据sessionKey获取到的sessionId。"+null);
        }
        if (id == null && WebUtils.isWeb(key)) {
        	if (log.isDebugEnabled()) {
        		log.debug("sessionId为null,sessionKey是WebSessionKey类型。");
            }
            ServletRequest request = WebUtils.getRequest(key);
            ServletResponse response = WebUtils.getResponse(key);
            id = getSessionId(request, response);
            if (log.isDebugEnabled()) {
        		log.debug("根据websessionKey获取request,respones，然后从request的cookie中获取session id");
            }
        }
        return id;
    }
